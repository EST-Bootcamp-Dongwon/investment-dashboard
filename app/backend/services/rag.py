...
# ── 말뭉치 판정 ──────────────────────────────────────────────────────────────
#
# **무엇이 말뭉치인가를 여기 한 곳에서 정한다.** 그 전까지는 같은 규칙(`docs/` 를
# 비재귀 `*.md` 로 훑는다)이 네 곳에 흩어져 있었다 — `routers/admin.py:_source_files` ·
# `scripts/index_docs_to_supabase.py` · `scripts/verify_rag_api.py` 의 두 glob.
# 네 벌이라 한 벌만 고치면 갈라지고, 실제로 그렇게 될 뻔했다.
#
# ## 왜 `README.md` 를 뺐는가 (2026-08-17)
#
# `docs/README.md` 가 들어오면서(커밋 b78562e) 말뭉치가 8개 → 9개, 청크가 178 → 179 가
# 됐다. `verify_rag_api.py` 의 "① 말뭉치 총 청크 수" 가 그때부터 실패했는데,
# 네트워크 그룹이라 `--offline` 실행에서 계속 건너뛰어져 드러나지 않았다.
#
# **상수를 179 로 올리는 것이 답이 아니다.** `docs/README.md` 는 이 모듈의 **문서
# 지도(라우터)** 다 — Diátaxis 4분류 표와 "작성 예정" 체크리스트가 본문의 전부다.
# 말뭉치로 인정하면 F27 이 투자 질문에 **문서 목차를 근거로 내민다.** 말뭉치는 투자
# 상식 문서이지 이 저장소의 안내문이 아니다(`docs/spec/` 45개를 이미 같은 이유로
# 뺐다 — 비재귀 glob 이 그 판단이었다).
#
# 규칙을 넓히지 않는다. 이름을 정확히 하나만 뺀다 — `docs/04.md` 처럼 내용으로
# 판단해야 하는 문서가 나중에 생기면 그때 여기에 이름을 더한다.
CORPUS_EXCLUDED = frozenset({"README.md"})


def is_corpus_doc(name: str) -> bool:
    """파일명이 말뭉치인가. 경로가 아니라 **파일명**을 받는다(`'07.md'`).

    `doc_chunk.source_doc` 이 파일명이므로 판정 단위도 파일명이어야 한다. 경로를
    받으면 호출부마다 `.name` 을 붙이는 자리가 생기고, 한 곳에서 빠뜨리면 판정이
    조용히 달라진다.
    """
    return name.endswith(".md") and name not in CORPUS_EXCLUDED
