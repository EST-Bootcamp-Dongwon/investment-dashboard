...
OPERATION_DOCS: dict[str, tuple[str, str, str]] = {
    ...
    "/api/rag/ask": ("RAG", "관련 학습 문서 찾기", "질문을 해시 임베딩(384차원)으로 바꿔 Supabase pgvector에서 관련 청크를 찾고, 검색된 원문만 정리해 답변합니다. **외부 AI를 부르지 않습니다** — 답변은 검색된 원문만 조립해 만듭니다(2026-08-16 폐기, 절대 제약 1). 응답에는 교육용이며 개인별 투자 조언이 아니라는 `disclaimer`가 항상 함께 나갑니다. 응답의 `followups`에는 검색된 원문의 제목·질문문장에서 규칙으로 뽑은 확인 질문이 최대 3개 실립니다. 질의와 어휘가 겹치는 것만 고르므로 겹치는 것이 없으면 빈 배열입니다. 색인이 비어 있으면 503입니다."),
