...
**서버 이식은 아래 3줄 형태를 씁니다** — 위 표와 완전히 등가이고 테스트 케이스를 쓰기 쉽습니다.
...
> **관찰 3가지.**
> ① `goal` 축은 사실상 두 가지 일만 합니다 — `protect` 면 고정, 그 외에는 **27칸 중 1칸**
> (`long`·`medium`)에서만 `growth`/`balance` 차이가 납니다.
...
>
> 규칙 자체를 바꿀지는 스펙 변경이라 여기서 정하지 않습니다. → [CN-029](../00-index/변경이력.md#cn-029)
...
`anon_id` 는 프런트에 이미 있는 `visitorId()`(`app.js:354~367`)를 재사용하면 됩니다 —
...
> 실제로 깨지지는 않습니다 — `randomUUID` 경로는 32자 고정입니다. 다만 폴백 경로
> (`app.js:357`)의 정확한 길이는 `Math.random().toString(36)` 결과에 좌우돼
> **(확인 필요)** 입니다. → [CN-030](../00-index/변경이력.md#cn-030)
...
응답은 공통적으로 `image_base64` 또는 `image` + 평가지표입니다.
...
| base64 차트 15곳 | `image` / `image_base64` | `dpi=130~140` PNG |
