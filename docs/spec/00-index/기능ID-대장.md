...
| ID | 기능 | SPA 라우트 | 백엔드 | 등급 | 상태 |
| --- | --- | --- | --- | --- | --- |
| **F03** | **포트폴리오 추천** — 목표·기간·하락 반응 → 안정/균형/성장 구성 | `portfolio-guide` | `POST /api/recommendation/preview`·`create`<br>`GET /api/recommendation/history`·`detail` | **A** | **동작** |
...
> ✅ **F03 의 백엔드가 생겼습니다** (2026-08-10, [CN-085](변경이력.md#cn-085)).
> A등급 4개 중 유일하게 백엔드가 없던 기능이었고, 추천 로직이 브라우저 안에만 있어
> 저장·재현·검증이 불가능했습니다 ([CN-011](변경이력.md#cn-011) 해소).
> 판정 규칙은 [CN-029](변경이력.md#cn-029) 점수제로 교체됐고,
> [CN-065](변경이력.md#cn-065) 3계층의 첫 적용 대상입니다.
>
> **[CN-047](변경이력.md#cn-047) 이 "프론트전용 6개" 를 F03 하나로 좁혔고, 그 하나가
> 닫혔습니다** — 나머지 5개는 학습·실습 화면이라 프론트전용이 옳습니다.
...
| [CN-039](변경이력.md#cn-039) 반영 | **52** | +4 | F03 백엔드화 — `/api/recommendation/` `preview`·`create`·`history`·`detail`. **2026-08-10 구현 완료** ([CN-085](변경이력.md#cn-085)) — 실측 **55** (51 + 4, 삭제 3건은 미착수) |
...
> A등급 4개 중 **F03 은 백엔드가 없었습니다.** v2.0 의 1순위 작업이었고,
> **2026-08-10 에 구현됐습니다** ([CN-085](변경이력.md#cn-085)).
...
| 1 | **프론트전용 기능이 6개** (F03·F12·F18·F19·F24·F25) | [CN-047](변경이력.md#cn-047) — **6개 중 F03 하나만 진짜 문제**. 나머지 5개는 학습·실습 화면이라 프론트전용이 옳음. **그 하나는 [CN-085](변경이력.md#cn-085) 로 해소** | **해소** |
